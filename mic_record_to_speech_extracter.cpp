...
int main(int argc, char *argv[]) 
{
	...
	unsigned int audio_length = 3; // second
	...
	char delimiter = '|'; // 受け取った音声認識結果と音源定位結果を区切る文字
	const char *recog_result_temp;
	char *recog_result;
	const char *localization_result;
	...
	while(true)
	{
		...
		// 受信データを音声認識結果と音源定位結果に分けて標準出力
		int display_counter = 0;
		// Pythonのリストの要素をfor文で取り出すのに似た処理
		for (const auto subStr : split(receive_data, delimiter)) 
		{
			if(display_counter==0)
			{
				recog_result_temp = subStr.c_str(); // const char型に変換
				// ポインタの参照先が音源定位結果で書き換えられてしまうためstrcpyでコピー
				strcpy(recog_result, recog_result_temp);
				cout << "音声認識結果：" << recog_result << endl;
			} else if(display_counter==1) 
			{
				localization_result = subStr.c_str();
				cout << "音源定位結果：" << localization_result << endl;
			}
			display_counter++;
		}
		// メモリの開放
...
// stringを特定の文字でsplitする関数
vector<string> split(string str, char delimiter) 
{
    int first = 0;
    int last = str.find_first_of(delimiter);
	// vectorは、長さを変えることのできる配列
    vector<std::string> result;
	// 区切り文字が見つかったところまで文字列を取得してvectorに追加
